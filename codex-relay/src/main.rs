...
struct Args {
    ...
    /// For very long tasks (hours), set this to a large value or 0.
    #[arg(long, env = "CODEX_RELAY_TOTAL_TIMEOUT_SECS", default_value = "600")]
    total_timeout_secs: u64,

    /// Per-chunk SSE read timeout in seconds (0 = no timeout).
    #[arg(long, env = "CODEX_RELAY_CHUNK_TIMEOUT_SECS", default_value = "180")]
...
async fn main() -> Result<()> {
    ...
    let state = AppState {
        ...
        client: build_client(args.total_timeout_secs),
...
/// Build an HTTP client with sensible timeouts so the relay never hangs
/// indefinitely on slow or unresponsive upstream APIs.
fn build_client(total_timeout_secs: u64) -> Client {
    let mut builder = Client::builder()
        .connect_timeout(Duration::from_secs(10))
        .pool_idle_timeout(Duration::from_secs(90));
    if total_timeout_secs > 0 {
        builder = builder.timeout(Duration::from_secs(total_timeout_secs));
    }
...
async fn handle_responses_inner(
    ...
    if req.stream {
        ...
        stream::translate_stream(stream::StreamArgs {
            ...
            request_messages,
            model,
            chunk_timeout_secs: state.chunk_timeout_secs,
        })
        .into_response()
    } else {
